...
;; Old version
#_(defn make-name-map [path names]
  (println "make-map-name:" names)
  (into {}
        (for [name names]
...
;; Cannot handle use statements with multiple sub-namespace
#_(defn parse-use-section [ns-sections]
  (println "parse-use-section(full):" (:use ns-sections))
  (apply merge
         (for [piece (cons ['clojure.core] (:use ns-sections))]
           (let [ns (str (if (sequential? piece)
                           (first piece)
                           piece))]
             (println "parse-use-section:" ns piece)
             (make-name-map ns
...
(defn parse-use-section [ns-sections]
  #_(println "parse-use-section(full):" (:use ns-sections))
  (apply merge
         (for [piece (cons ['clojure.core] (:use ns-sections))]
           (let [ns (str (if (sequential? piece)
                           (first piece)
                           piece))]
             #_(println "parse-use-section:" ns piece)
             (doseq [el (if (and (sequential? piece)
                                 (> (count piece) 1))
                          (rest piece)
                          [(get @ns-names ns)])]               
               (make-name-map ns el))))))
